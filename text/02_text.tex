...
Používajú dva datasety: CUAVE - obsahuje nahrávky reči pri pohľade spredu aj z profilu a LIVER - obsahuje nahrávky vyslovovania holandského slova "liver" pri pohľade spredu.
...
Kvôli testovaniu na rôznych datasetoch trénovali sieť pre rôzne počty bodov (21, 68, ...) na tvári. 
...
V hlavičkovom súbore sú definované 2 triedy: \texttt{VVAD} a  \texttt{Output}.
Trieda \texttt{VVAD} je hlavnou triedou, ktorá obsahuje verejne metódy:
\begin{itemize}
\item \texttt{Frame} - hlavná metóda, ktorej vstupom je video snímok a výstupom je objekt triedy \texttt{Output}.
\item \texttt{FrameForLearningThreshold} - metóda sa volá na začiatku používania knižnice na určenie prahu. Vstupom metódy je video snímok a výsledným výstupom je prah, pomocou ktorého metóda \texttt{Frame} vyhodnocuje, či vo videosekvencii nasala reč alebo nie.
...
\subsubsection{Detekcia tváre pomocou knižnice Dlib}
Ako sme popísali v časti \ref{OpenCVvsDlibNadpis}, na detekciu tváre sme použili knižnicu Dlib, konkrétne detekciu tváre pomocou HoG. 
...
Dôvodom je to, že predtým
Tvár sa po nájdení v obraze posiela na 

\subsubsection{Vytvorenie modelu pre detekciu bodov na tvári}
...
Zo stránky \cite{ibug} sme získali dataset 300-W (pri použití daného datasetu žiadajú na stránke citovať \cite{sagonas2016300}, \cite{sagonas2013300} a \cite{sagonas2013semi}) obsahujúci fotografie z datasetov afw, helen, ibug a lfpw. 
Dataset samozrejme obsahoval aj testovaci a trénovací xml súbor s označenými umiestneniami 68 bodov na tvári, pomocou ktorých je možné natrénovať presnejší model.
